...
class AppStore extends Component {
  ...
  getActiveTabId = () => {
    ...
    const filteredApps = appsList.filter(
      eachAppListDetails => eachAppListDetails.category === activeTabId,
    )
    return filteredApps
  }
  ...
  render() {
    ...
    const filteredApps = this.getActiveTabId(searchResult)
    return (
      <div className="app-bg-container">
        <div className="app-store-const">
          ...
          <ul className="tab-list-const">
            {tabsList.map(eachtabDetails => (
              <TabItem
                key={eachtabDetails.tabId}
                eachtabDetails={eachtabDetails}
                setCategoryTabId={this.setCategoryTabId}
              />
            ))}
          </ul>
          <ul className="app-item-const">
            {filteredApps.map(eachAppItemDetails => (
              <AppItem
                key={eachAppItemDetails.appId}
                eachAppItemDetails={eachAppItemDetails}
              />
            ))}
          </ul>
        </div>
